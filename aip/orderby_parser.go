...
func ParseOrderBy(text string) ([]OrderBy, error) {
	...
	if err != nil {
		return nil, errors.WithMessagef(errors.WithStack(err), "syntax error")
	}

	result := make([]OrderBy, len(expr.SortOrder))
	...
	for _, orderBy := range result {
		if _, ok := uniqueFieldPaths[orderBy.FieldPath.String()]; ok {
			return nil, errors.Errorf("field appears multiple times: %q", orderBy.FieldPath)
		}
